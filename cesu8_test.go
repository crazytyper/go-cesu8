...
func TestEncodeDecode(t *testing.T) {
	...
	var testcases = []testcase{
		...
		{"abcd","abcd"},
		{"☺☻☹","☺☻☹"},
		{"日a本b語ç日ð本Ê語þ日¥本¼語i日©","日a本b語ç日ð本Ê語þ日¥本¼語i日©"},
		{
			"日a本b語ç日ð本Ê語þ日¥本¼語i日©日a本b語ç日ð本Ê語þ日¥本¼語i日©日a本b語ç日ð本Ê語þ日¥本¼語i日©",
			"日a本b語ç日ð本Ê語þ日¥本¼語i日©日a本b語ç日ð本Ê語þ日¥本¼語i日©日a本b語ç日ð本Ê語þ日¥本¼語i日©",
		},
		{"Hello 👋","Hello \xED\xA0\xBD\xED\xB1\x8B"},
	}

	for i, tc := range testcases {
		if a := EncodeString(tc.UTF8); string(a) != tc.CESU8 {
			t.Fatalf("%d encode mismatch - expected: %X, actual: %X", i, []byte(tc.CESU8), a)
		}
		if a := DecodeString([]byte(tc.CESU8)); a != tc.UTF8 {
			t.Fatalf("%d decode mismatch - expected: %X, actual: %X", i, []byte(tc.UTF8), []byte(a))
		}
	}
}
